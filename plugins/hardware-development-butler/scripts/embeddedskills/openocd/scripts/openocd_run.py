...
def main() -> None:
    ...
    gate = safety_gate.check_token(
        args.action,
        args.confirm_token,
        target=target or board or "",
        probe=interface or board or "OpenOCD",
        voltage=args.voltage,
        current_limit=args.current_limit,
        erase_scope=args.erase_scope or (args.mode if args.action == "erase" else ""),
        recovery=args.recovery,
        external_loads=args.external_loads,
        artifact=args.safety_artifact or file_path or "",
    ...
    if not gate["allowed"]:
        ...
        if args.as_json:
            ...
            print(f"[{args.action}] failed - {blocked['error']['message']}", file=sys.stderr)
        sys.exit(1)
